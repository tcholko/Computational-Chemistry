...
int main(int argc, char* argv[]) {
   string sdcd, strfr, ligin, ligline, recin, recline, graline, selec_cut, siconc, slj_cut, sscaleLJ, sbeg, sfreq, send;
   if (argc == 11) { sdcd = argv[1]; ligin = argv[2]; recin = argv[3]; selec_cut=argv[4]; siconc = argv[5]; slj_cut=argv[6]; sscaleLJ=argv[7];
                     sbeg=argv[8]; sfreq=argv[9]; send=argv[10]; }  
   else { cout << "Usage: ./E3.0 traj.dcd ligand.pqr receptor.pqr elec_cutoff ionic_conc (M) vdw_cutoff LJ_scale_coef (0 to 1) begin freq end" << endl;
...
// Calculating electrostatic and Lennard-Jones potentials for all ligand-receptor atom pairs    
   ...
   for (int frame = beg; frame < end; frame+=freq) {
     ...
     cout << "Frame " << frame << ": ";
     count++;
     for (int lai = 0; lai < natom; lai++) {
   
      for (int rai = 0; rai < nrecatom; rai++) {
        float dx2 = pow(coords[lai][0][frame]-recx[rai], 2);
        float dy2 = pow(coords[lai][1][frame]-recy[rai], 2);
        float dz2 = pow(coords[lai][2][frame]-recz[rai], 2);
