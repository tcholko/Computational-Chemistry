...
/// Begin calculation of surface residence times ///
   ...
   for (int i; i=0; i++) { eventrestime[i] = 0; }
   ...
   for (int n=0; n<nrep; n++) {
    //tdwell = 0; 
     for (int f=0; f<nframe/freq; f++) {
      ...
      if (adsorbed == 0) { 
         //if (tdwell > 0.0) { eventrestime[e] = tdwell; e++; cout << "event res time = " << eventrestime[e]; }
  ...
  cout << ">> Total surface res time: " << ttot/1e6 << " us, Num unique adsorptions: " << nassoc << ", Avg res time: " << ttot/(nassoc*1e6) << " us\n";
  double avg = ttot/nassoc; 
  double sumd2, sd;
  for (int i=0; i<nassoc; i++) { 
     sumd2 += pow(avg - eventrestime[i], 2);
     cout << "event res time: " << eventrestime[i]/1e6 <<" us\n";
    }
   sumd2 /= nassoc;
   sd = sqrt(sumd2);   
  cout << "SD residence time: " << sd/1e6 << " us\n"; 
return 0;
}
