...
int main(int argc, char* argv[]) {
   ...
   for (int i=0;i<nframe/freq;i++) {
     ...
     dcd.read((char*)&tmp, sizeof(float)); // I think this may be simplified if do dcd.seekg instead of dcd.read, but make sure it
     dcd.read((char*)&tmp, sizeof(float)); // reads forward from current spot not from beg of file
     dcd.read((char*)&tmp, sizeof(float));
   ...
   for (int n=COMatom; n<=COMatom+((nrep-1)*ligatoms); n+=ligatoms) {
     cout << "** Finding binders for replicate " << currentrep << endl << "-------------------------------\n";
     for (int m=0; m<(nframe/freq)-1; m++) {
       float dz = coords[n-1][m+1]-coords[n-1][m];
       if (dz > 300 /*&& coords[n-1][m] < 150*/) {   //Try changing dz if # binders doesn't match log
         if (coords[n-1][m-20] < 40) { twoD++; } else { threeD++; } 
         cout << "> Binder at frame " << m+1 << " comatom= " << n << "\n2D binders: "<<twoD<<" 3D binders: "<<threeD<<" Total: "<<twoD+threeD<<"\n";   
       }
      }
     currentrep++; 
     cout << endl;
