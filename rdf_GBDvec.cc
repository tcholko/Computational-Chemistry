...
///// Read an input file with atom numbers of ref pts and prx atoms, etc. /////   
  ...
  while (inf.is_open()) {
   ...
   for (int j=0; j<nmol; j++) { 
     mol[j] = atof(lines[j+11].c_str());  // Reads in RDF moleclue atom nums
   ...
   string recline;
   //cout << "vector 2nd item is " << vec[1] << endl;
   ...
   for (int i=0; i<1266; i++){
   getline (recpqr, recline);
   rec[0][i].push_back(atof(recline.substr(33, 7).c_str()));
   rec[1][i].push_back(atof(recline.substr(43, 7).c_str()));
   rec[2][i].push_back(atof(recline.substr(53, 7).c_str()));
   }
...
///// Reading DCD trajectory ////// 
   ...
   dcd.close();
   delete[] buffer;
   int bins[nmol][22]; // Create our bins
   ...
   for (int j=begframe; j<endframe; j+=freq) {
     ...
     for (int k = 0; k<nmol; k++) { 
       ...
       nmeasured++;
       for (int l=0; l<1266; l++) {
         double dx2 = pow((rec[0][l]-coords[currentmol][0][j]), 2);
         double dy2 = pow((rec[1][l]-coords[currentmol][1][j]), 2);
         double dz2 = pow((rec[2][l]-coords[currentmol][2][j]), 2);
