...
/// Reading DCD trajectory ///
   ...
   for (int i=0; i<30; i++) { 
    ...
    //cout << ">> Bin " << i+1 << ": Total: " << totbin[i] << " || " 
    avgbin[i] = totbin[i]/framesread;
    //cout << " Average: " << avgbin[i] <<  endl;
   }
   ...
   for (int i=0; i<30; i++) {
     float sum_d2=0;
      for (int j=begframe; j<begframe+framesread; j++) {
      float frame_d2 = pow((frame_totbin[j][i] - avgbin[i]), 2);
      sum_d2 += frame_d2;
     }
      float SD = sqrt(sum_d2/(framesread-1));
      cout << ">> Bin " << i+1 << " Total: " << totbin[i] << " | Average: " << avgbin[i] << "+/- " << SD << endl;
    }
